...
from src.utils.metrics import eval_metrics, compute_metrics_retrieval, evaluate_linear_probe
import wandb
import json
import os
import pickle
...
def retrieve_evaluate_RAC_(
    ...
    if args.Faiss_GPU and hasattr(faiss, "StandardGpuResources"):
        res = faiss.StandardGpuResources()
        index = faiss.index_cpu_to_gpu(res, 0, index)
        ...
        train_feats_np = train_feats.detach().cpu().numpy().astype("float32")
        evaluate_feats_np = evaluate_feats.detach().cpu().numpy().astype("float32")
